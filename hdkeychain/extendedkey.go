...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := bchec.S256().ScalarBaseMult(k.key)
		pubKey := bchec.PublicKey{Curve: bchec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(bchec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilx, ily := bchec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := bchec.ParsePubKey(k.key, bchec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := bchec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := bchec.PublicKey{Curve: bchec.S256(), X: childX, Y: childY}
		childKey = pk.SerializeCompressed()
	}

	// The fingerprint of the parent for the derived child is the first 4
	// bytes of the RIPEMD160(SHA256(parentPubKey)).
	parentFP := bchutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a bchec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*bchec.PublicKey, error) {
	return bchec.ParsePubKey(k.pubKeyBytes(), bchec.S256())
}

// ECPrivKey converts the extended key to a bchec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*bchec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := bchec.PrivKeyFromBytes(bchec.S256(), k.key)
	return privKey, nil
}

// Address converts the extended key to a standard bitcoin pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*bchutil.AddressPubKeyHash, error) {
	pkHash := bchutil.Hash160(k.pubKeyBytes())
	return bchutil.NewAddressPubKeyHash(pkHash, net)
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(bchec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(bchec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := bchec.ParsePubKey(keyData, bchec.S256())
		if err != nil {
			return nil, err
		}
	}

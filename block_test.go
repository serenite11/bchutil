...
func TestBlock(t *testing.T) {
	b := bchutil.NewBlock(&Block100000)
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := bchutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := bchutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := bchutil.OutOfRangeError(wantErr)
	...
	_, err = bchutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(bchutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, bchutil.OutOfRangeError(""))
	}

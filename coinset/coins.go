...
func (cs *CoinSet) TotalValue() (value bchutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue bchutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue bchutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount bchutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue bchutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue bchutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue bchutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue bchutil.Amount, coins []Coin) (Coins, error) {
...
// bchutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() bchutil.Amount {
	return bchutil.Amount(c.txOut().Value)
}
